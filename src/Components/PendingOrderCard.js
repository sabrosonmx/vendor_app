...
const PendingOrderCard = ({
  ...
  //----------------- _Rejcted Reason state----------------------
  const [rejectReason, setRejecReason] = useState(false);
  const [rejectReasonText, setRejecReasoneText] = useState('');
  const [rejectData, setRejectData] = useState({});

  console.log(rejectReason, 'rejectReasonText');
  const styles = stylesFunc({fontFamily, themeColors});

  //---------------------_Rejcted Reason -----------------
  const _onRejectReason = (rejectdata, status) => {
    if (data?.id == rejectdata?.id) {
      setRejecReason(true);
      setRejectData(rejectdata);
    } else {
      setRejecReason(false);
    }
  };

  const onSubmit = () => {
    if (rejectReasonText) {
      updateOrderStatus(rejectData, 8, rejectReasonText);
    } else {
      alert(strings?.ENTER_REASON_FOR_REJECTING_ORDER);
    }
  };
  return (
    <TouchableOpacity
            ...
            {rejectReason ? (
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <View
                  ...
                  <TextInput
                    placeholder={`${strings?.ENTER} ${strings?.REJECT_REASON}`}
                    style={{
                      paddingVertical: moderateScaleVertical(13),
                      paddingLeft: moderateScale(5),
                    }}
                    onChangeText={(text) => setRejecReasoneText(text)}
                  />
                </View>
                <View
                  style={{
